...
async function loadCandidateViews(
  ...
  return candidateRows.map((row) => {
    ...
    return {
      ...
      fallbackLabel: restaurant.fallback_label ?? "Restaurant",
      source: row.source,
...
export async function listAdminPolls(): Promise<AdminPollSummary[]> {
  ...
  const [pollResponse, candidateResponse, ballotResponse] = await Promise.all([
    ...
    supabase.from("poll_candidates").select("poll_id,is_active"),
    supabase.from("ballots").select("poll_id,is_submitted"),
  ]);
...
export async function listWinnerHistory(): Promise<WinnerHistoryView[]> {
  ...
  const { data, error } = await supabase
    .from("winner_history")
    .select("id,restaurant_id,source_poll_id,source,won_on,notes,restaurants!inner(id,google_place_id,fallback_label)")
    .order("won_on", { ascending: false });
