...
export function PlaceAutocompleteSearch({
  ...
  return (
    <div className="place-search space-y-3">
      <div className="place-search-field" ref={containerRef} />
      <p
        className={
          hasError
            ? "flex items-center gap-2 rounded-xl border border-warning/30 bg-gold-soft px-3 py-2 text-sm text-warning"
            : "sr-only"
        }
        aria-live="polite"
        role="status"
      >
        {hasError ? <WarningIcon className="shrink-0" size={16} /> : null}
        {status}
      </p>
